import requests
import hashlib
import sys


def request_api_data(query):
    url = 'https://api.pwnedpasswords.com/range/' + query
    response = requests.get(url)
    if response.status_code != 200:
        raise RuntimeError(
            f'Error fetching: {response.status_code}, check the API and try again')
    return response


def get_password_leaks(hashes, hash_to_check):
    # This is a generator function
    hashes = (line.split(':') for line in hashes.text.splitlines())

    for h, count in hashes:
        if h == hash_to_check:
            return count
    return 0


def pwned_api_check(password):
    sha1password = hashlib.sha1(password.encode('utf-8')).hexdigest().upper()
    first5, tail = sha1password[:5], sha1password[5:]
    response = request_api_data(first5)

    return get_password_leaks(response, tail)
    # We pass this function the list of hashes returned
    # by the API (response) as well as the tail, which
    # we need to check which (if any) of the hashes is
    # our password


def main(args):
    for password in args:
        count = pwned_api_check(password)
        if count:
            print(f'The password {password} has been hacked {count} times')
        else:
            print(
                f"Congratulations, the password {password} has never been hacked!")

    return "Done!"


sys.exit(main(sys.argv[1:]))
